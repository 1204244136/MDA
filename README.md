...
`scripts/phone-display-16x9.ps1` 会用 `wm size` 把逻辑分辨率临时改成**竖屏形状的 16:9**（1080x1920，游戏横屏旋转后正好是 1920x1080），并且在**宿主程序退出后自动还原**，不会把手机留在异常分辨率上。

用法：在 MXU 里把它加为**前置程序**（配置页 → 前置程序 → 添加），把 `<...>` 换成你自己的值（序列号可用 `adb devices` 查看第一列）：

| 字段         | 值                                                                                                                                                |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 程序         | `pwsh.exe`                                                                                                                                        |
| 参数         | `-NoProfile -ExecutionPolicy Bypass -File "<MDA 目录>\scripts\phone-display-16x9.ps1" -Adb "<adb.exe 路径>" -Serial "<设备序列号>" -KeepScreenOn` |
| 等待退出     | 勾选                                                                                                                                              |
| 已运行时跳过 | 不勾（adb 命令秒退，无需跳过）                                                                                                                    |

不放心可以先手动还原一次确认效果：

```powershell
pwsh -File "<MDA 目录>\scripts\phone-display-16x9.ps1" -Adb "<adb.exe 路径>" -Serial "<设备序列号>" -Restore
```

切记：

- 尺寸**不要填 `1920x1080`**：横屏形状的覆盖值在竖屏面板上会让游戏画面错位
- `wm size` 的覆盖值**不跨重启**；脚本退出时会自动还原，守护进程被强杀时重启手机即可恢复
- 脚本不含任何固定路径与序列号，全部由参数传入
